...
using UnityEngine.XR.Interaction.Toolkit.Samples.StarterAssets;
    ...
    /*
        Cette classe est la classe GoalManager sans les elements que nous n'utilisons pas
...
public struct CustomGoal
{
    ...
    /// </summary>
    public CustomGoalManager.CustomOnboardingGoals CurrentGoal;
    ...
    public CustomGoal(CustomGoalManager.CustomOnboardingGoals goal)
    {
        CurrentGoal = goal;
        Completed = false;
    }
...
public class CustomGoalManager : MonoBehaviour
{
    ...
    /// </summary>
    public enum CustomOnboardingGoals
    {
        /// <summary>
        /// Current empty scene
        /// </summary>
    ...
    void Update()
    {
        if (Pointer.current != null && Pointer.current.press.wasPressedThisFrame && !m_AllGoalsFinished && (m_CurrentGoal.CurrentGoal == CustomOnboardingGoals.FindSurfaces || m_CurrentGoal.CurrentGoal == CustomOnboardingGoals.Hints || m_CurrentGoal.CurrentGoal == CustomOnboardingGoals.Scale))
        {
    ...
    void CompleteGoal()
    {
        if (m_CurrentGoal.CurrentGoal == CustomOnboardingGoals.TapSurface)
    ...
    void PreprocessGoal()
    {
        if (m_CurrentGoal.CurrentGoal == CustomOnboardingGoals.FindSurfaces)
        {
            m_CurrentCoroutine = StartCoroutine(WaitUntilNextCard(5f));
        }
        else if (m_CurrentGoal.CurrentGoal == CustomOnboardingGoals.Hints)
        {
            m_CurrentCoroutine = StartCoroutine(WaitUntilNextCard(6f));
        }
        else if (m_CurrentGoal.CurrentGoal == CustomOnboardingGoals.Scale)
        {
            m_CurrentCoroutine = StartCoroutine(WaitUntilNextCard(8f));
        }
        else if (m_CurrentGoal.CurrentGoal == CustomOnboardingGoals.TapSurface)
        {
            m_SurfacesTapped = 0;
            m_ObjectSpawner.objectSpawned += OnObjectSpawned;
        }
    ...
    {
        m_SurfacesTapped++;
        if (m_CurrentGoal.CurrentGoal == CustomOnboardingGoals.TapSurface && m_SurfacesTapped >= k_NumberOfSurfacesTappedToCompleteGoal)
        {
            CompleteGoal();
        }
    }
    ...
    public void StartCoaching()
    {
        ...
        {
            var findSurfaceGoal = new CustomGoal(CustomOnboardingGoals.FindSurfaces);
            m_OnboardingGoals.Enqueue(findSurfaceGoal);
        }

        int startingStep = m_AllGoalsFinished ? 1 : 0;

        var tapSurfaceGoal = new CustomGoal(CustomOnboardingGoals.TapSurface);
        var translateHintsGoal = new CustomGoal(CustomOnboardingGoals.Hints);
        var scaleHintsGoal = new CustomGoal(CustomOnboardingGoals.Scale);
        var rotateHintsGoal = new CustomGoal(CustomOnboardingGoals.Hints);
